...
% ---- Abstract -----------------------------------------------
\begin{abstract}
Incorporating large language models (LLMs) in medical questions answering 
...
We further find that the only clinically fine-tuned model
(MedGemma\,1.5\,4B) underperforms the larger general-purpose models
on both quality and reproducibility, challenging the assumption that
domain specialization justifies smaller scale.
...
is available at 
https://github.com/aviad-buskila/llm\_medical\_reproducibility/tree/v1.0.0.
\end{abstract}
...
highly susceptible to the rapid spread of misinformation and unverified advice. 
...
\begin{enumerate}
  ...
  \item \textbf{Empirical results} on three small open-weight models
        ...
        the near-deterministic temperature regime and that clinical
        fine-tuning alone does not compensate for smaller scale.
\end{enumerate}
...
despite its clinical fine-tuning, suggesting that, at 4B parameters,
domain adaptation does not compensate for the capacity gap relative
to its 12B Gemma sibling.
...
we would also like to emphasis that we did not use $T{=}0.0$ not 
only because it is not common for questions answering GenAI tasks, 
but also as a gatekeeper of training biased in case the dataset 
...
relative to Gemma\,3\,12B---a general-purpose model with 3$\times$
the parameters and no domain adaptation.
On every metric in Tables~\ref{tab:quality} and~\ref{tab:repro},
MedGemma finishes last or second-to-last.
This is consistent with findings in the broader literature that
scale often dominates domain adaptation when the target task is
covered in pretraining~\cite{singhal2023large}.
Practitioners should not assume that a clinically-branded or
fine-tuned model will outperform a larger general-purpose one,
particularly when parameter counts differ by more than 2$\times$.
...
Second, \textbf{scale outweighs domain fine-tuning at small parameter
counts}.
Gemma\,3\,12B (general-purpose) outperforms MedGemma\,1.5\,4B
(clinically fine-tuned) on every dimension: judge score, lexical
...
Qwen-Med), evaluation on USMLE and MedQA benchmarks, ensemble
